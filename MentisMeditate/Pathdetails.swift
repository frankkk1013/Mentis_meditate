...
struct Pathdetails: View {
    ...
    @State var sheetToView : SheetViewCard = SheetViewCard(subtitle: "", title: "", backgroundImage: Image(systemName: "is"), briefSummary: "", description: "", fromPathdetails: true, color: "", titleExercise: "", progress: UseProgress(), duration: "3 min")
            ...
            VStack {
                ...
                HStack{
                    ...
                    Button {
                        ...
                        Text(NSLocalizedString(NSLocalizedString("Add", comment: ""), comment: ""))
                ...
                ForEach(MentisPaths.paths[id].week, id: \.self.id){ it in
                    VStack(alignment: .leading){
                        ...
                        ItemRow(text: it.title, day: it.day, color: Color(MentisPaths.paths[id].title)).contentShape(Rectangle())
                            .onTapGesture {
                                sheetToView = SheetViewCard(subtitle: "idk", title: MentisPaths.paths[id].title, backgroundImage: Image(MentisPaths.paths[id].title), briefSummary: it.nameExercise, description: it.description,fromPathdetails: true, color: MentisPaths.paths[id].color
                                                            , titleExercise: it.title, progress: progress, duration: it.duration)
                            showingSheet.toggle()
